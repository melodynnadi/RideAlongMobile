...
export async function createSetupIntent(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/create-setup-intent`, {
    method: 'POST',
...
export async function savePaymentMethod(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/save-payment-method`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(request),
  });
...
export async function setDefaultPaymentMethod(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/payment-methods/${encodeURIComponent(paymentMethodId)}/default`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(request),
  });
...
export async function createRidePayment(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/payments/create-intent`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
  ...
  // Save payment to Firestore as a client-side backup in case the server write fails
  if (result.id && firestore) {
    try {
      ...
      console.warn('[createRidePayment] Client Firestore backup failed:', firestoreError);
    }
  }
...
export async function cancelRidePayment(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/payments/cancel-intent`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function refreshCustomer(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/payments/refresh-customer`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function captureRidePayment(
  ...
  const response = await fetchWithTimeout(`${API_BASE_URL}/api/payments/capture-ride-payment`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
