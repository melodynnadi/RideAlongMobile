    ...
    <SafeAreaView className="flex-1 bg-bg">
      <View className="flex-1 px-6 pt-8">
        ...
        <View className="bg-card border border-border rounded-3xl p-6 mb-4">
          ...
          <Text className="text-muted">{driverProfile?.email}</Text>
          {driverProfile?.university && (
            <Text className="text-muted text-sm mt-1">{driverProfile.university}</Text>
          )}
          {driverProfile?.rating !== undefined && (
            <Text className="text-warning text-sm mt-2">★ {driverProfile.rating.toFixed(1)}</Text>
          )}
        </View>
