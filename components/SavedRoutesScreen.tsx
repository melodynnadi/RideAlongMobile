    ...
    <View style={styles.root}>
      <SafeAreaView style={styles.safe} edges={['top', 'left', 'right']}>
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={loading} onRefresh={() => void loadRoutes()} tintColor={ORANGE} />}
        >
          ...
          <View style={styles.introCard}>
            <View style={styles.introIcon}><Ionicons name="notifications-outline" size={21} color={ORANGE} /></View>
          ...
          {loading && routes.length === 0 ? (
            <View style={styles.stateCard}><ActivityIndicator color={ORANGE} /><Text style={styles.stateText}>Loading saved routes...</Text></View>
          ) : routes.length === 0 ? (
            <View style={styles.emptyState}>
              <View style={styles.emptyIcon}><Ionicons name="git-branch-outline" size={27} color={ORANGE} /></View>
              ...
              {routes.map((route) => {
                ...
                return <View key={route.id} style={[styles.routeCard, duplicateRouteId === route.id && styles.routeCardDuplicate]}>
                  ...
                  <View style={styles.routeActions}>
                    <TouchableOpacity style={styles.iconButton} onPress={() => openEdit(route)} accessibilityLabel="Edit saved route"><Ionicons name="pencil-outline" size={18} color={NAVY} /></TouchableOpacity>
                    <TouchableOpacity style={[styles.iconButton, styles.deleteIconButton]} onPress={() => confirmDelete(route)} accessibilityLabel="Delete saved route"><Ionicons name="trash-outline" size={18} color="#C94747" /></TouchableOpacity>
                  </View>
                </View>;
              })}
            </View>
          ...
          <View style={styles.modalSheet}>
            ...
            >
              <View style={styles.modalHeader}>
                ...
                <TouchableOpacity style={styles.modalClose} onPress={closeModal} hitSlop={hitSlop}><Ionicons name="close" size={20} color={NAVY} /></TouchableOpacity>
              </View>
              ...
              <TouchableOpacity style={[styles.saveButton, isSaving && styles.buttonDisabled]} onPress={() => void save()} disabled={isSaving}>
                {isSaving ? <ActivityIndicator color="#FFFFFF" /> : <Text style={styles.saveButtonText}>{editing ? 'Update route' : 'Save route'}</Text>}
              </TouchableOpacity>
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  introCard: { borderRadius: 18, backgroundColor: '#FFF2E9', flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 16, marginBottom: 24 },
  introIcon: { width: 40, height: 40, borderRadius: 13, backgroundColor: '#FFFFFF', alignItems: 'center', justifyContent: 'center' },
  introTitle: { color: NAVY, fontSize: 15, lineHeight: 20, fontWeight: '700', marginBottom: 3 },
  introText: { color: '#687386', fontSize: 12, lineHeight: 18 },
  sectionHeader: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 12 },
  sectionTitle: { color: NAVY, fontSize: 19, lineHeight: 25, fontWeight: '700' },
  routeCount: { color: MUTED, fontSize: 12, lineHeight: 17, marginTop: 2 },
  addButton: { minHeight: 42, borderRadius: 21, backgroundColor: ORANGE, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingHorizontal: 15 },
  ...
  emptyIcon: { width: 58, height: 58, borderRadius: 20, backgroundColor: '#FCEBDD', alignItems: 'center', justifyContent: 'center', marginBottom: 14 },
  emptyTitle: { color: NAVY, fontSize: 18, lineHeight: 24, fontWeight: '700', textAlign: 'center' },
  emptyText: { color: MUTED, fontSize: 13, lineHeight: 19, textAlign: 'center', marginTop: 6, maxWidth: 290 },
  emptyButton: { minHeight: 44, borderRadius: 22, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 20, marginTop: 18 },
  emptyButtonText: { color: '#FFFFFF', fontSize: 13, fontWeight: '700' },
  routeList: { gap: 12 },
  routeCard: { minHeight: 154, borderRadius: 18, borderWidth: 1, borderColor: BORDER, backgroundColor: '#FFFFFF', flexDirection: 'row', padding: 16 },
  routeCardDuplicate: { borderColor: '#E6A15D' },
  routeRail: { width: 18, alignItems: 'center', paddingTop: 8, paddingBottom: 9, marginRight: 10 },
  originDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: NAVY },
  routeLine: { flex: 1, width: 1, borderLeftWidth: 1, borderStyle: 'dashed', borderColor: '#CAD0D8', marginVertical: 5 },
  destinationDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: ORANGE },
  routeCopy: { flex: 1, paddingRight: 8 },
  locationLabel: { color: '#99A2B1', fontSize: 9, lineHeight: 13, letterSpacing: 1.2, fontWeight: '700' },
  destinationLabel: { marginTop: 12 },
  locationText: { color: NAVY, fontSize: 14, lineHeight: 19, fontWeight: '600', marginTop: 2 },
  locationSecondary: { color: MUTED, fontSize: 12, lineHeight: 17, marginTop: 2 },
  routeActions: { justifyContent: 'space-between', marginLeft: 4 },
  iconButton: { width: 38, height: 38, borderRadius: 13, backgroundColor: PAPER, alignItems: 'center', justifyContent: 'center' },
  deleteIconButton: { backgroundColor: '#FFF1F1' },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(21,35,58,0.35)', justifyContent: 'flex-end' },
  modalSheet: { maxHeight: '92%', borderTopLeftRadius: 26, borderTopRightRadius: 26, backgroundColor: BG, paddingTop: 10, overflow: 'hidden' },
  ...
  modalSubtitle: { color: MUTED, fontSize: 12, lineHeight: 18, marginTop: 3 },
  ...
  fieldLabel: { color: '#8B94A6', fontSize: 10, lineHeight: 15, letterSpacing: 1.3, fontWeight: '700', marginBottom: 7 },
  autocompleteTop: { minHeight: 48, zIndex: 20, marginBottom: 16 },
  autocompleteBottom: { minHeight: 48, zIndex: 10, marginBottom: 24 },
  saveButton: { minHeight: 54, borderRadius: 27, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center', zIndex: 1 },
